  ...
  return (
    <div className="space-y-8 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold tracking-tight">Exercise <span className="text-gradient">Library</span></h1>
          <p className="text-muted-foreground mt-1">Explore {exercises.length} high-intensity movements verified by AI.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:border-primary/50 transition-all w-64"
            />
          </div>
        ...
        {muscleGroups.map((m) => (
          <button
            ...
            className={cn(
              ...
              activeMuscle === m 
                ? "bg-primary text-primary-foreground border-primary shadow-[0_0_15px_rgba(0,242,254,0.3)]" 
      ...
      <div className="glass p-8 rounded-[2rem] border border-primary/20 relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-30 transition-all duration-500">
          <Zap size={140} className="text-primary rotate-12" />
        </div>
        <div className="relative z-10">
          <h2 className="text-3xl font-bold mb-2">Build a Custom Session</h2>
          ...
          <button 
            ...
            className="bg-primary text-primary-foreground px-8 py-4 rounded-2xl font-bold flex items-center gap-3 hover:scale-105 transition-all shadow-[0_0_30px_rgba(0,242,254,0.4)] disabled:opacity-50"
          >
            {isGenerating ? (
              <>
      ...
      <div>
        <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6">
          <div>
            <h2 className="text-2xl font-bold">Featured Exercises</h2>
            <p className="text-xs text-muted-foreground mt-1">Verified movements for optimal hypertrophy</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-bold uppercase tracking-widest bg-white/5 px-3 py-1 rounded-full border border-white/10 text-muted-foreground">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredExercises.map((ex, i) => (
              <motion.div
                ...
                whileHover={{ y: -10 }}
                className="glass rounded-[2rem] overflow-hidden border border-white/5 group cursor-pointer"
              >
                <div className="h-56 overflow-hidden relative">
                  ...
                  <div className="absolute bottom-4 left-4">
                    <span className="px-3 py-1 rounded-full bg-primary/20 backdrop-blur-md text-[10px] font-bold uppercase tracking-widest text-primary border border-primary/20">
                      {ex.muscleGroup}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  ...
                  <h3 className="text-xl font-bold mb-2 group-hover:text-primary transition-colors line-clamp-1">{ex.name}</h3>
                  <p className="text-xs text-muted-foreground line-clamp-2 mb-6 leading-relaxed min-h-[32px]">
                    {getExerciseDescription(ex)}
                  </p>
                  <button className="w-full py-3.5 rounded-2xl bg-white/5 border border-white/10 group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-primary transition-all font-bold flex items-center justify-center gap-2 text-sm">
                    Start Exercise
